...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " l15-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  l15-tx [options] <hex-tx> [commands]  Update hex-encoded transaction\n"
                "or:     l15-tx [options] -create [commands]   Create hex-encoded transaction\n"
                "\n";
